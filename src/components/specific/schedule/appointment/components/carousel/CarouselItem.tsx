...
// Chiều cao card mặc định
const ITEM_HEIGHT = 420;

// Component hiển thị từng item trong carousel
const CarouselItem: React.FC<CarouselItemProps> = ({ item, scrollX, index }) => {
  const step = SNAP_INTERVAL;

  // Lấy các style animation cho card, text, image dựa trên scrollX và index
...
const styles = StyleSheet.create({
  cardContainer: {
    width: ITEM_WIDTH,                 // Chiều ngang card
    height: ITEM_HEIGHT - 250,         // Chiều cao card
    overflow: 'hidden',                // Không hiển thị tràn
  ...
  imageBackgroundStyle: {
    width: '100%',
    height: '100%',
    resizeMode: 'cover',               // Ảnh phủ toàn card
    overflow: 'hidden',
  },
  overlay: {
    ...StyleSheet.absoluteFillObject,  // Phủ toàn bộ card
    backgroundColor: 'rgba(0,0,0,0.35)', // Overlay mờ tối
  },
  textContainer: {
    position: 'absolute',
    bottom: 0,
    left: 0,
    right: 0,
    padding: SIZES.padding,           // Khoảng cách từ viền
  },
